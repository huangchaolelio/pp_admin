...
**版本更改**: 1.0.0 → 1.1.0
**递增理由**: 新增一条原则「VI. 后端业务流程对齐」以及相应的治理合规条款，
属于 MINOR 级别的新增原则/实质性扩展指导，对既有原则无破坏性重定义。
...
- ✅ `.specify/templates/plan-template.md` — 章程检查门控新增"业务流程对齐"一项
- ✅ `.specify/templates/tasks-template.md` — 完善阶段新增"业务流程对齐验证"任务示例
- ✅ `CODEBUDDY.md` — 运行时指导新增业务流程参考文档链接
- ✅ `.specify/templates/spec-template.md` — 通用模板已覆盖，无需改动（假设段示例仍适用）
...
本后台管理系统的所有系统设计、页面结构、交互流程与后续优化 MUST 与后端主项目的业务流程文档保持对齐，权威来源为
[char_pp/docs/business-workflow.md](https://github.com/huangchaolelio/char_pp/blob/master/docs/business-workflow.md)。
在开展以下任一活动前，作者 MUST 先核对该文档中对应的业务流程：

- 新增或调整菜单、页面信息架构与操作入口
- 设计表单字段、状态机、作业（Job/Task）列表列与详情展示
- 定义前端触发的异步任务（视频预处理 / KB 抽取 / 教学提示抽取等）交互与跳转关系
- 对既有流程的用户体验优化（例如按钮迁移、合并/拆分页面、改变默认筛选）

若页面实现与后端业务流程存在分歧，MUST 在 PR 描述中显式标注分歧点并选择以下之一：
(a) 调整前端以对齐后端流程；(b) 在 PR 中附上推动后端调整的跟进 Issue 链接，并在前端保留
TODO 注释。禁止在无任何记录的情况下让前端长期偏离后端流程。

**理由**: 本管理系统的主要价值来自忠实呈现并驱动 char_pp 的后端业务流程；一旦前后端对流程的理解出现隐性分歧，
会直接导致运营看到的数据口径、作业状态、操作入口与真实后端行为不一致，修复成本远高于事前对齐成本。
...
涉及页面信息架构、作业/任务交互或业务流程重排的 PR，审查者 MUST 额外核对其与
`char_pp/docs/business-workflow.md` 的一致性（原则 VI），并在评审意见中显式确认。

**版本**: 1.1.0 | **批准日期**: 2026-04-23 | **最后修订**: 2026-04-30
