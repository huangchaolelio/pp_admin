...
- [ ] **原则 IV 安全访问控制**: 计划是否明确新增/变更接口的鉴权与 RBAC 角色归属?
- [ ] **原则 V 数据可观测性**: 计划是否声明关键操作的日志/指标埋点与后台可视化位置?
- [ ] **原则 VI 后端业务流程对齐**: 计划是否已核对
      [char_pp/docs/business-workflow.md](https://github.com/huangchaolelio/char_pp/blob/master/docs/business-workflow.md) ?
      - 新增/调整菜单、页面、入口的映射关系
      - 作业 (Job/Task) 状态机、列表列与详情字段的一致性
      - 异步任务触发、跳转与完成态的流程对齐
      - 如与后端流程存在分歧, 已在本计划 "复杂度跟踪" 章节中登记
